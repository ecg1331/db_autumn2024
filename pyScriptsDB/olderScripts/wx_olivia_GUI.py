...
class DBInterface(wx.Frame):
    ...
    def load_svg(self, svg_file_path):
        """Load an SVG file using wx.svg.SVGImage and convert it to wx.Bitmap."""
        try:
            # Load the SVG file using wx.svg.SVGImage
            svg_image = wx.svg.SVGImage()
            if svg_image.LoadFile(svg_file_path):  # Load the SVG file
                # Convert to wx.Bitmap
                bitmap = svg_image.ConvertToBitmap()
                return bitmap
            else:
                print(f"Error: Failed to load SVG file {svg_file_path}")
                return None
        except Exception as e:
